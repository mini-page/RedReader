...
class _ReaderScreenState extends ConsumerState<ReaderScreen> {
            ...
            GestureDetector(
              ...
              child: Column(
                children: [
                  ...
                  const Spacer(),
                  // The Reading View
                  _buildReaderView(state, settings.fontSize, Color(settings.orpColorValue), settings.fontFamily),
                  const Spacer(),
  ...
  Widget _buildAppBar(BuildContext context, ReaderState state) {
    final progressPercent = state.tokens.isEmpty ? 0 : (state.index / state.tokens.length * 100).toInt();
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildCircleButton(Icons.tune_rounded, _toggleSettings, isSelected: _showSettingsPopup),
        ],
      ),
    );
  }

  Widget _buildCircleButton(IconData icon, VoidCallback onTap, {bool isSelected = false}) {
    return Container(
      ...
      child: IconButton(
        icon: Icon(icon, color: isSelected ? Colors.black : Colors.white, size: 20),
        onPressed: onTap,
      ),
    );
  }

  Widget _buildReaderView(ReaderState state, double fontSize, Color orpColor, String fontFamily) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          Container(height: 12, width: 2, color: Colors.white.withOpacity(0.2)),
  ...
  Widget _buildBottomControls(ReaderState state, ReaderController controller) {
    final progress = state.tokens.isEmpty ? 0.0 : (state.index / state.tokens.length).clamp(0.0, 1.0);
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              IconButton(icon: const Icon(Icons.first_page_rounded, color: Colors.white, size: 28), onPressed: controller.previous),
              IconButton(icon: const Icon(Icons.fast_rewind_rounded, color: Colors.white, size: 28), onPressed: controller.skipBackward),
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle,
                    boxShadow: [
                      BoxShadow(color: Colors.white.withValues(alpha: 0.1), blurRadius: 30, spreadRadius: 5),
                    ],
                  ),
                  child: Icon(
              ...
              IconButton(icon: const Icon(Icons.fast_forward_rounded, color: Colors.white, size: 28), onPressed: controller.skipForward),
              IconButton(icon: const Icon(Icons.last_page_rounded, color: Colors.white, size: 28), onPressed: controller.next),
            ],
          ),
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                        ...
                        return Padding(
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: isSel ? Colors.white : Colors.white.withOpacity(0.05),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              alignment: Alignment.center,
                              child: Text('$s word${s > 1 ? 's' : ''}', 
                                style: TextStyle(
                                  color: isSel ? Colors.black : Colors.white.withOpacity(0.7), 
                                  fontSize: 13, 
                                  fontWeight: FontWeight.bold
                                )
                              ),
                  ...
                  const SizedBox(height: 24),
                  _buildPopupSectionTitle('FONT SIZE (${settings.fontSize.toInt()}pt)'),
                  _buildThickSlider(
    ...
    return Row(
      children: [
        IconButton(
          ...
          icon: Icon(Icons.remove_circle_outline_rounded, color: Colors.white.withOpacity(0.3), size: 20),
        ...
        IconButton(
          visualDensity: VisualDensity.compact,
          icon: Icon(Icons.add_circle_outline_rounded, color: Colors.white.withOpacity(0.3), size: 20),
          onPressed: () => onAdjust(25),
        ),
      ],
    );
  ...
  Widget _buildPopupToggle(String title, bool value, ValueChanged<bool> onChanged) {
    return ListTile(
      contentPadding: EdgeInsets.zero,
      visualDensity: VisualDensity.compact,
      title: Text(title, style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 13, fontWeight: FontWeight.w500)),
  ...
  Widget _buildPopupFontSelector(BuildContext context, AppSettings settings, SettingsController ctrl) {
    return GestureDetector(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text(font, style: GoogleFonts.getFont(font, color: Colors.white, fontSize: 16)),
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(settings.fontFamily, style: GoogleFonts.getFont(settings.fontFamily, color: Colors.white, fontSize: 14)),
...
class _WordView extends StatelessWidget {
  ...
  const _WordView({required this.word, required this.orpIndex, required this.fontSize, required this.orpColor, required this.fontFamily});
